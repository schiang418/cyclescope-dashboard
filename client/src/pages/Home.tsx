...
export default function Home() {
  ...
  const handleDownloadAll = async () => {
    try {
      toast.info("Preparing ZIP file with all 18 charts...");
  ...
  const handleUpdateAll = async () => {
    ...
    try {
      ...
      if (result.success) {
        setCacheKey(Date.now()); // Force reload all images
        toast.success(result.message);
        
        // Refetch last update timestamp
        await refetchLastUpdate();
        
        // Show updated timestamp in toast
        if (result.lastUpdate) {
          const formattedTime = formatTimestamp(result.lastUpdate);
          toast.info(`Charts updated at ${formattedTime}`);
        }
      } else {
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="border-b border-border">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-foreground">CycleScope</h1>
            <p className="text-sm text-muted-foreground">Market Analysis Dashboard</p>
            {lastUpdateData && (
              <div className="mt-1 space-y-0.5">
                ...
                {(lastUpdateData as any).lastFullSuccess && (
                  <p className="text-xs text-green-600 dark:text-green-400">
                    ✓ Last 18/18 success: {formatTimestamp((lastUpdateData as any).lastFullSuccess)}
                  </p>
                )}
              </div>
            )}
